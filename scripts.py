...
hours_repers = (10, 15, 18, 21)

now = datetime(2025,10,31, 10,42)

real_dedline = now + timedelta(days=1)
print(f'hours_repers = {hours_repers}\n'
      f'now = {now}\n'
      f'real_dedline = {real_dedline}')
...
if reper_dedline < real_dedline:
    while reper_dedline < real_dedline:
        for hour in hours_repers:
            reper_dedline = datetime(real_dedline.year, real_dedline.month, real_dedline.day, hour) + timedelta(days=day)
            if reper_dedline > real_dedline:
                break
        if reper_dedline < real_dedline:
            day += 1

    print(f'reper_dedline = {datetime(2025,10,19).weekday()}')
print(f'\nfinish_dedline = {reper_dedline}')