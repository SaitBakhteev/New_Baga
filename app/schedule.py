...
async def check_payment_dedline(notify=False, bot: Bot = None):
    try:
        now = datetime.now()
        if notify is False:
            ...
            event_user = await (EventUser.filter(event__payment_dedline__lte=now + timedelta(hours=1)).
                                select_related('event', 'user').order_by('event_id'))
        if event_user:
        ...
        await logger.info(f'DoesNotExist in check_pyment_dedline: {e}')
        stream_logger.info(f'DoesNotExist: {e}')
    except DBConnectionError as e:
        await logger.info(f'DBConnectionError in check_pyment_dedline: {e}')
...
async def test_for_sch():
    try:
        ev = await Event.all().order_by('id').first()
        print(ev.event_text)
        print(230)
        text = ev.event_text
        print(232)
        await Event.create(user_id=5,
                           created_at = datetime.now(),
                           event_datetime=datetime.now()+timedelta(seconds=100),
                           payment_dedline=datetime.now()+timedelta(seconds=10),
                           event_text = 'text',
                           training_type = "тип трени")
        print(237)
        last_event = await rqs.get_event(for_schedule=True, last_record=True)
        print(f'last_event = {last_event}')
        print(240)
        # payment_dedline, id = last_event['payment_dedline'], last_event['id']
        # for i i
    except Exception as e:
        stream_logger.info(f'Ошибка в test_for_sch: {e}')
