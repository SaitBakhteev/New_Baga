...
from config import bot, setup_logger, TRAINING_TYPES, DEDLINE_TYPE, REPER_HOURS, NUMBERS
...
async def ntf(message: Message, state: FSMContext):
    ...
    text = ('Включенные уведомления дают Вам значительные преимущества, покольку:\n'
         '🔹 Вам будут приходить напоминания об оплате за час до дедлайна\n'
         '🔹 При переходе из резерва в основной список бот сразу же Вам сообщит об этом\n'
         '🔹 Вы сразу же будете получать сообщения о новых тренировках\n\n')
    await message.answer(text, parse_mode='HTML', reply_markup= await kb.notify(receive_notifications))
...
async def show_events(message: Message, state: FSMContext, is_admin: bool):
    data = await state.get_data()
...
async def choose_event(call_mess: Message | CallbackQuery, state: FSMContext,
    ...
    try:
        ...
        text = trn_info['text']
        if user_cache[call_id].receive_notifications is False:
            text += ('\n\n <b>⚡️Внимание! </b> \nУ вас выключены уведомления🔕. Это означает, что:\n'
         '✖️ Вы не будете получать напоминание об оплате за час до дедлайна\n'
         '✖️ Вы не будете получать уведомления о переходе из резерва в основной список\n'
         '✖️ Вы не будете получать уведомления о новых тренировках\n\n'
         '<b>Рекомендуется</b>💡 включить уведомления /ntf')
        if isinstance(call_mess, CallbackQuery):
...
async def delete_from_training_confirm(message: Message, state: FSMContext, is_admin: bool):
    if message.text.lower().strip() == 'да':
        ...
        text = (f"Пользователь с никнеймом @<i>{message.from_user.username}</i> удалился из следующей тренировки\n\n"
...
async def add_dedline_and_finish(call: CallbackQuery | Message, state: FSMContext, is_admin: bool):
    ...
    try:
        ...
        if 'is_update' not in data:  # если создается новая тренировка (там работает CallbackQuery)
            ...
            now = datetime.now()
            # now = datetime(2025, 10, 15, 19, 32)  # заглушка для теста
