...
class Event(Model):  # модель создаваемых тренирвок
    ...
    remind_is_sended = fields.BooleanField(null=True)  # напоминание о голосовании
...
# Голосование
class Voting(Model):
    '''Изначально эта модель предназначалась для других целей. Но раз уж у SQLite ограничения и переименовать не можем,
    то просто будем иметь в виду, что модель для рассылки уведомлений прлписчиками по результатам голосования. По этой
    причине поле question не соответствует своему названия, на самом деле это текст рассылки уведомлений подписчикам'''
    question = fields.TextField()
    training_type = fields.TextField(null=True)
