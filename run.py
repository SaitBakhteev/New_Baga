...
async def startup(dispatcher: Dispatcher):
    try:
        ...
        scheduler.add_job(delete_events, CronTrigger(hour=2, minute=00), id="work_by_stats_and_delete_events_at_2")
        scheduler.add_job(delete_events, CronTrigger(hour=5, minute=00), id="work_by_stats_and_delete_events_at_5")
        for i in REPER_HOURS:  # это планировщик по проверкам дедлайнов
            scheduler.add_job(check_payment_dedline, CronTrigger(hour=i - 1, minute=0), id=f"notify_by_{i}")
